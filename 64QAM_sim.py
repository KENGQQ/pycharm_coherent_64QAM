...
xpart, ypart = 1, 0
eyestart, eyeend = 3, 4
tap1_start, tap1_end ,tap1_scan= 59, 61, 2 ;    tap2_start, tap2_end = 1, 1;     tap3_start, tap3_end, tap3_scan = 9, 11, 4
cma_stage= [1, 2, 3]; cma_iter = [100, 20, 30]
isrealvolterra = 0
iscomplexvolterra = 0
##################### intialize  #####################
window_length = 7000
...
for eyepos in range(eyestart, eyeend, 1):
    ...
    ##########IQimba################
    ######### CD ################
    cd_compensator = CD_compensator(Rx_Signal_X)
    Rx_Signal_X_CD = cd_compensator.FIR_CD();
    Rx_Signal_X_CD = Rx_Signal_X_CD[Rx_Signal_X_CD != 0]
    # Histogram2D('Rx_X_CDcomp_{}'.format(eyepos), Rx_Signal_X_CD, Imageaddress)
    ...
    for tap_1 in range(tap1_start, tap1_end, tap1_scan):
        ...
        cma.stepsize_x = cma.stepsizelist[8]  ; CMAstage1_stepsize_x = cma.stepsize_x;
        cma.stepsize_y = cma.stepsizelist[8]  ; CMAstage1_stepsize_y = cma.stepsize_y;
        ...
        for taps_3 in range(tap3_start, tap3_end, tap3_scan):
            ...
            cma.stepsize_x = cma.stepsizelist[14]  ; CMAstage3_stepsize_x = cma.stepsize_x;
            cma.stepsize_y = cma.stepsizelist[14]  ; CMAstage3_stepsize_y = cma.stepsize_y;
            ...
            if isplot == True:
                ...
                Histogram2D('CMA_Y_{}_stage3 taps={} {}'.format(eyepos, cma.cmataps, cma.type),
                            Rx_Y_CMA_stage3, Imageaddress)
...
#--------------------------- X PART-------------------------------------------------
        ...
        # --------------------------- Y PART------------------------
            if ypart == True:
                ...
                PN_RxY = ph.QAM_64QAM_1(phasenoise_RxY, r1_o=1.55, r3_i=3.1, r3_o=3.47, r9_i=7.94)
